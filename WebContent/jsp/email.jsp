...
<body >
	<div class="login_cont" style="width: 700px">
		<div class="login_nav"style="width: 700px;">
				<font size="5">邮箱验证</font>
		</div>
		<div class="alert alert-info" style="width:700px;font-size: 17px" id="beforHint">点击下面按钮发送邮件，确保您的邮箱真实有效。</div>  
		<div class="alert alert-info" style="width:700px;font-size: 17px" id="afterHint">邮件已发送至您的邮箱请注意查收</div>                  
        <div class="item col-xs-12 f-mb10" style="height:auto">
           <span class="intelligent-label f-fl" style="font-size: 20px">邮箱：${parameter.email[0]}</span> 
        </div>

        <form id="form" action="${pageContext.request.contextPath}/login.user?username=${user.username}&password=${user.password}" >
        ...
        <div class="mb2" style="margin-top: 30px" >
			...
			<input type="submit" style="color: #FFFFFF" value="注册" id="login" class="truesub" />
			<input type="button" style="display:none;line-height: 20px;font-size: 20px;margin-top: 20px" value="注册" class="fakesub" />
	...
	<script type="text/javascript">
		$(function(){
			...
			var usn = getUrlParam("username");
			var psw = getUrlParam("password");
			...
			$("#verifyYz").click(function(){
				...
				$.ajax({
					url:"${pageContext.request.contextPath}/sendEmail.user",
					data:{"email":email},
					success:function(data){
			...
			$("#login").click(function(){
				$.ajax({
					...
					data:{"email":email,"username":usn,"password":psw,"nickname":nickname,"sex":sex},
					success:function(){
					},
					dataType:"json",
					async:false
